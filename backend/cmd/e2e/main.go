...
func main() {
	...
	status, data = doReq("POST", "http://localhost:8080/api/admin/sellers", map[string]any{
		"brandName":         fmt.Sprintf("Super Store %d", uniqueId),
		"description":       "A great store",
		"contactEmail":      fmt.Sprintf("store_%d@super.com", uniqueId),
		"contactPhone":      fmt.Sprintf("+%d", uniqueId),
		"ownerName":         "Store Owner",
		"ownerEmail":        fmt.Sprintf("owner_%d@super.com", uniqueId),
	...
	status, data = doReq("POST", "http://localhost:8080/api/auth/login", map[string]string{
		"email":    fmt.Sprintf("owner_%d@super.com", uniqueId),
	...
	catSlug := fmt.Sprintf("electronics-%d", uniqueId)
	...
	brandSlug := fmt.Sprintf("techbrand-%d", uniqueId)
	...
	status, data = doReq("POST", "http://localhost:8080/api/seller/products", map[string]any{
		"title":      fmt.Sprintf("Wireless Mouse %d", uniqueId),
		"categoryId": catId,
		"brandId":    brandId,
		"priceCents": 15000,
		"currency":   "RUB",
	...
	if variantId == "" {
		fmt.Println("Variant ID is empty, could not proceed!")
		os.Exit(1)
	}
	...
	status, data = doReq("POST", "http://localhost:8080/api/auth/register", map[string]string{
		"name":     "Other Customer",
		"email":    fmt.Sprintf("othercust_%d@test.com", uniqueId),
		"password": "custPassword123",
	}, "")
	status, data = doReq("POST", "http://localhost:8080/api/auth/login", map[string]string{
		"email":    fmt.Sprintf("othercust_%d@test.com", uniqueId),
	...
	status, data = doReq("POST", "http://localhost:8080/api/admin/sellers", map[string]any{
		"brandName":         fmt.Sprintf("Other Store %d", uniqueId),
		"description":       "A great store",
		"contactEmail":      fmt.Sprintf("other_%d@super.com", uniqueId),
