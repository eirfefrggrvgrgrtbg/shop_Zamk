...
func (r *Repository) CreateSeller(ctx context.Context, s *Seller) error {
	...
	_, err := r.db.Exec(ctx, query,
		s.ID, s.BrandName, s.Slug, s.Description, s.ContactEmail, s.ContactPhone, s.Status, s.CreatedAt, s.UpdatedAt,
	)
	if err != nil {
...
func (r *Repository) GetSellerByID(ctx context.Context, id uuid.UUID) (*Seller, error) {
	query := `
		SELECT id, brand_name, slug, description, contact_email, contact_phone, status, created_at, updated_at
		FROM sellers
		WHERE id = $1
	`
	var s Seller
...
func (r *Repository) GetSellerByUserID(ctx context.Context, userID uuid.UUID) (*Seller, *SellerUser, error) {
	query := `
		SELECT s.id, s.brand_name, s.slug, s.description, s.contact_email, s.contact_phone, s.status, s.created_at, s.updated_at,
