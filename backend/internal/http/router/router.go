...
func New(
	...
	r.Route("/api/customer", func(r chi.Router) {
		...
		r.Get("/returns/{id}/messages", returnsHandler.GetCustomerReturnMessages)
		r.Post("/returns/{id}/messages", returnsHandler.SendCustomerReturnMessage)
		r.With(uploadLimit).Post("/returns/{id}/messages/attachments", returnsHandler.UploadCustomerReturnMessageAttachment)
	...
	r.Route("/api/admin", func(r chi.Router) {
		...
		r.With(adminDangerousLimit, perm("inventory.adjust")).Post("/inventory/reconciliations/{id}/resolve", inventoryHandler.ResolveReconciliationCase)
		...
		r.With(perm("warehouse.receiving")).Post("/fulfillments/{id}/receiving/scan-item", fulfillmentHandler.ScanReceivingItem)
