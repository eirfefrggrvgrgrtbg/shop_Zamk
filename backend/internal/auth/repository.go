...
func (r *Repository) CreatePasswordResetRequest(ctx context.Context, email string, userID *uuid.UUID) error {
	query := `
		INSERT INTO password_reset_requests (id, email, user_id, status)
		VALUES ($1, $2, $3, 'pending')
	`
	_, err := r.db.Pool.Exec(ctx, query, uuid.New(), email, userID)
	if err != nil {
