...
func (h *Handler) PackFulfillment(w http.ResponseWriter, r *http.Request) {
	...
	res, err := h.svc.PackFulfillment(r.Context(), adminID, fulfillmentID)
	if err != nil {
		if errors.Is(err, ErrFulfillmentNotFound) {
			h.writeError(w, http.StatusNotFound, "fulfillment_not_found", "Fulfillment not found")
			return
		}
		if errors.Is(err, ErrPackingNotAllowed) {
			h.writeError(w, http.StatusConflict, "packing_not_allowed", "Packing is not allowed for this fulfillment or order state")
			return
		}
	...
	if h.auditRepo != nil {
		...
		_ = h.auditRepo.RecordAudit(r.Context(), staff.AuditEvent{
			...
			EntityID:    &fid,
			Metadata:    staff.SanitizeMetadata(map[string]any{"fromStatus": "assembling", "toStatus": "packed", "actorRole": "admin"}),
		})
	}
