...
* **Problem**: A multi-seller order breaks this global status model. Each seller-scoped portion needs independent fulfillment state even though ZAMK owns the physical workflow. If an order contains items from two sellers, both items otherwise share the same shipment and global order status, preventing accurate status reflections per seller.
...
**Recommendation**: **Option B**. It provides exact control for ZAMK dispatch of each seller-scoped fulfillment and guarantees 1-to-1 or 1-to-N relationships explicitly tied to that fulfillment group, which is crucial for production tracking.

## 4. Status Model
* **Fulfillment Statuses (per seller-scoped group, operated by ZAMK)**: `awaiting_payment`, `paid`, `assembling`, `packed`, `shipped`, `delivered`, `cancelled`, `returned`, `refunded`.
...
* **Allowed**: Sellers can read their own order and fulfillment progress where a Seller read projection is exposed.
* **Denied**: Sellers cannot mutate fulfillment or parent-order status, pick, pack, create shipments, dispatch, mark delivery, access other sellers' fulfillments, or access buyer payment intents. ZAMK/Admin owns the physical workflow; the system and courier callbacks own automatic transitions.
...
## 8. Seller UI Impact
* **SellerOrders**: The list shows only items belonging to the seller and remains read-only. A future Seller projection may expose sanitized fulfillment progress without physical-operation controls or unnecessary customer PII.
...
* **Step 3**: Switch backend APIs so Seller reads and Admin/ZAMK operations use `order_fulfillments`.
...
* **Step 5**: Deploy Admin, Seller, and Buyer UI updates to consume the new grouped structures within their ownership boundaries.
* **Step 6**: Keep physical fulfillment commands under ZAMK/Admin ownership.
...
**Seller API**:
* Current Seller order reads are exposed under `/api/seller/orders`.
* `/api/seller/fulfillments` is not mounted.
* A future read-only fulfillment projection must not expose Seller fulfillment commands.
...
* **Phase C2 (API Read Layer)**: Domain and Admin read endpoints were added; a dedicated Seller fulfillment projection is future work.
* **Phase C3 (Seller Fulfillment Visibility)**: Seller order visibility remains read-only; a dedicated sanitized fulfillment projection is future work.
...
* [x] Phase C7: The obsolete Seller fulfillment action contract was removed; ZAMK/Admin owns assembling and packing.
* [x] Phase C8: Backend Order Creation creates fulfillments for all new orders. Payment confirmation moves fulfillments to `paid`. Legacy and backfilled orders remain fully supported. `NOT NULL` enforcement remains future work.
* [x] Phase C9: Schema Hardening & Integrity Guards. Enforced `NOT NULL` on `order_items.order_fulfillment_id`. Verified DB health. Added guardrails in backend `service.go` to ensure atomic creation. Legacy shipment relationships preserved.
* [x] Phase C10: Shipment Fulfillment Hardening. Enforced `shipments.fulfillment_id` as `NOT NULL`. Updated audit scripts and backend guardrails. Included backfill logic for safe single-fulfillment legacy orders. Note: `shipments.fulfillment_id` is now NOT NULL; legacy order-level shipment endpoint remains only as a compatibility wrapper; it must resolve fulfillment_id before insert; `shipments.order_id` remains for compatibility/reference; NULL shipment fulfillment is no longer valid.
