...
* [x] Phase C8: Backend Order Creation creates fulfillments for all new orders. Payment confirmation moves fulfillments to `paid`. Seller actions strictly depend on `paid` status. Legacy and backfilled orders remain fully supported. `NOT NULL` enforcement remains future work.
