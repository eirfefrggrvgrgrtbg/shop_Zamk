...
import { getAdminPackingQueue, PackingQueueItem } from '../api/adminPicking';
import { formatOrderNumber } from '../utils/orderFormatters';

export function AdminPackingQueue() {
  ...
  const fetchQueue = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await getAdminPackingQueue();
      setItems(data);
    } catch (err: any) {
      setError(err.message || 'Не удалось загрузить очередь упаковки.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div data-testid="packing-queue-page" className="space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Очередь упаковки</h1>
          <p className="text-sm text-gray-500 mt-1">
            Заказы, полностью укомплектованные на складе и готовые к упаковке
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="p-3.5 rounded-xl border bg-white border-gray-200 shadow-sm">
          <div className="text-xs text-gray-500 font-medium">Ожидают упаковки</div>
          <div className="text-xl font-bold mt-1 text-gray-900" data-testid="packing-queue-count">
            {totalOrders}
          </div>
        </div>
        <div className="p-3.5 rounded-xl border bg-white border-gray-200 shadow-sm">
          <div className="text-xs text-gray-500 font-medium">Всего единиц к упаковке</div>
      ...
      {isLoading && !error && (
        <div className="p-12 text-center text-gray-500 bg-white rounded-xl border border-gray-200 shadow-sm">
          ...
          <p className="text-sm font-medium">Загрузка очереди упаковки...</p>
        </div>
      )}
      ...
      {!isLoading && !error && items.length === 0 && (
        <div className="p-12 text-center bg-white rounded-xl border border-gray-200 shadow-sm space-y-3">
          ...
          <h3 className="text-base font-bold text-gray-900">Нет заказов, ожидающих упаковки</h3>
          <p className="text-xs text-gray-500 max-w-md mx-auto">
            Все укомплектованные заказы уже упакованы, либо текущие заказы находятся на этапе сборки.
          </p>
        </div>
      )}
      ...
      {/* Queue List */}
      {!isLoading && !error && items.length > 0 && (
        <div className="space-y-3" data-testid="packing-queue-list">
          {items.map((item) => {
            const readyTimestamp = formatTimestamp(item.pickingCompletedAt || item.createdAt);

            return (
              <div
                key={item.fulfillmentId}
                onClick={() => navigate(`/fulfillment/packing/${item.fulfillmentId}`)}
              ...
              >
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500">
                    ...
                    <span className="flex items-center gap-1 text-emerald-700 font-medium">
                      ...
                      Сборка завершена ({item.pickedQuantity} / {item.totalQuantity})
                    </span>
                    {readyTimestamp && (
                      <span className="flex items-center gap-1 text-gray-400">
                        <Clock className="w-3.5 h-3.5" />
                        Готов с {readyTimestamp}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center sm:self-center shrink-0">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(`/fulfillment/packing/${item.fulfillmentId}`);
                    }}
