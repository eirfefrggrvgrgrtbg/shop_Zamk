...
import { REVIEW_STATUS_LABELS, REVIEW_STATUS_STYLES } from '../components/orders/reviewHelpers';
          ...
          {reviews.map((review) => (
            ...
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                ...
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${REVIEW_STATUS_STYLES[review.status] || 'bg-graphite/5 dark:bg-white/10 text-ash'}`}>
              ...
              {(review.comment || review.text) && (
                <p className="text-sm text-ash dark:text-white/70">{review.comment || review.text}</p>
              )}
              {review.moderationComment && review.status === 'rejected' && (
                <div className="mt-3 p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs text-rose-700 dark:text-rose-400">
