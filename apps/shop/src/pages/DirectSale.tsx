...
export function DirectSale() {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        ...
        const [productsRes, apiCategories, apiBrands] = await Promise.all([
          fetchDirectSaleProducts(params),
          fetchCategories(),
          fetchBrands(),
        ]);
        ...
        console.error('Failed to load direct sale products:', err);
    ...
    loadProducts();
  }, [activeCategory, activeBrand, priceRange, sortBy, searchParams, setSearchParams]);
    ...
    <div className="relative z-10 min-h-screen pt-24 md:pt-28 pb-20">
      <div className="container mx-auto px-4 sm:px-6 max-w-[1400px]">
        ...
        <nav className="flex items-center gap-2 text-sm text-ash mb-6">
          ...
          <span className="text-graphite dark:text-white">Вещи ZAMK</span>
        </nav>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-ash mt-1">{totalProducts} товаров из платформенной продажи</p>
          </div>
