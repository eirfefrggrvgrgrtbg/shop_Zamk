        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isRegister && (
            <div className="space-y-1">
              <div className="flex gap-1 h-1 w-full rounded-full overflow-hidden bg-black/5 dark:bg-white/10">
                {[1, 2, 3, 4, 5].map((level) => (
                  <div 
                    key={level} 
                    className={`h-full flex-1 transition-colors duration-300 ${strength >= level ? getStrengthColor() : 'bg-transparent'}`}
                  />
                ))}
              </div>
              <p className="text-[11px] text-graphite/50 dark:text-white/50 px-1 leading-tight">
                Пароль должен содержать от 10 символов, строчные и заглавные буквы, цифры и спецсимволы.
              </p>
            </div>
          )}
