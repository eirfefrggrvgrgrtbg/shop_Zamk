      ...
      <div className='relative z-10 min-h-screen pt-28 pb-20'>
        <div className='container mx-auto px-4 sm:px-6 max-w-5xl'>
          <HeroBlock
            ...
            description='Добавь вещи из каталога, чтобы продолжить оформление в единой студийной системе.'
            right={
              <EmptyState
                icon='cart'
                title='Нет товаров'
    ...
    <div className='relative z-10 min-h-screen pt-28 pb-20'>
      <div className='container mx-auto px-4 sm:px-6 max-w-[1280px]'>
        <HeroBlock
          ...
          description={`${items.length} товаров в подборке. Всё оформлено в мягких rounded-панелях без табличной сухости.`}
        />

        <div className='mt-10 grid grid-cols-1 lg:grid-cols-12 gap-6'>
          <div className='lg:col-span-8 space-y-4'>
            {items.map((item) => (
              <article key={item.product.id} className='glass-panel p-4 md:p-5 flex gap-4'>
                <Link to={`/product/${item.product.id}`} className='h-28 w-24 overflow-hidden rounded-2xl border border-border-lighter'>
